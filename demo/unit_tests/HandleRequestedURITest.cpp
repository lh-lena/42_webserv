...
TEST(ServerTest, HandleRequestedURI)
{
  ...
  server.handleRequestedURI(base_path6, path6);
  ...
  server.handleRequestedURI(base_path7, path7);
  ...
  server.handleRequestedURI(base_path8, path8);
  EXPECT_EQ(path8, expected_path8);

}

TEST(ServerTest, handleErrorPageResponse)
{
  std::vector<Location> locations = server.getLocations();

  int st_code1 = 405;
  std::string path1 = server.handleErrorPageResponse(st_code1, locations[0]);
  std::string expected_path1 = "var/www/error_pages/405.html";
  EXPECT_EQ(path1, expected_path1);
  
  int st_code2 = 404;
  std::string path2 = server.handleErrorPageResponse(st_code2, locations[0]);
